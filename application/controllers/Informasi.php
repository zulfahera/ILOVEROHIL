...
class Informasi extends CI_Controller
{
    ...
    { 
        ...
        $this->load->view("informasi/vw_informasi", $data);
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view("layout/header", $data);
            $this->load->view("informasi/vw_ubah_informasi", $data);
            $this->load->view("layout/footer");
        } else {
            $data = [
                'universitas' => $this->input->post('universitas'),
            ...
            $this->Informasi_model->update(['idinformasi' => $id], $data);
            $this->session->set_flashdata('message', '<div class="m-alert m-alert--outline m-alert--outline-2x alert alert-success alert-dismissible fade show" role="alert">
            <button type="button" class="close" data-dismiss="alert" aria-label="Close">
            </button>
            <strong>Yeay!!!</strong> Data Informasi Beasiswa berhasil di Ubah
        </div>');
            redirect('Informasi');
        }
    }
